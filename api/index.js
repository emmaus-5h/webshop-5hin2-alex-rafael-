...
function getProducts(request, response) {
  ...
  let data = []
  const sqlOpdracht = db.prepare('SELECT products.id AS id, products.name AS name, products.description AS description, products.code AS code, products.price AS price, products.rating_id AS rating FROM products JOIN Category ON products.id = Category.product_id WHERE Category.id = ? ORDER BY products.id ASC');
...
function getProductById(request, response) {
  ...
  data = sqlOpdracht.all(product.id) 
  response.status(200).send(data)
}
